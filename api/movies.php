...
if ($method === 'GET') {
    ...
    $query = "SELECT * FROM movies WHERE status != 'archived' ORDER BY 
              CASE 
                WHEN release_date > '$today' THEN 0 
                ELSE 1 
              END, 
              release_date DESC LIMIT $offset, $limit";
    $result = $conn->query($query);
...
// Add movie (Admin)
    ...
    if ($action === 'add') {
        ...
        $query = "INSERT INTO movies (title, description, poster_image, release_date, duration, genre, director, rating, status) 
                  VALUES ('$title', '$description', '$poster_image', '$release_date', $duration, '$genre', '$director', $rating, '$status')";
    ...
    // Update movie
        ...
        $query = "UPDATE movies SET title = '$title', description = '$description', poster_image = '$poster_image', 
                  ...
                  rating = $rating, status = '$status' WHERE id = $id";

        if ($conn->query($query)) {
            sendResponse(true, 'Movie updated successfully');
        } else {
            sendResponse(false, 'Failed to update movie', null, 500);
        }
    }
